	...
	YAHOO.util.Dom = {
		...
		setXY : function(P, S, R) {
			var Q = function(V) {
				...
				var W = [ parseInt(this.getStyle(V, "left"), 10),
		...
		getDocumentHeight : function() {
			var Q = (M.compatMode != "CSS1Compat") ? M.body.scrollHeight
					: M.documentElement.scrollHeight;
			var P = Math.max(Q, B.Dom.getViewportHeight());
			return P;
		},
		getDocumentWidth : function() {
			var Q = (M.compatMode != "CSS1Compat") ? M.body.scrollWidth
		...
		getViewportHeight : function() {
			...
			var Q = M.compatMode;
			if ((Q || G) && !C) {
				P = (Q == "CSS1Compat") ? M.documentElement.clientHeight
						: M.body.clientHeight;
			}
			return P;
		},
		getViewportWidth : function() {
			...
			var Q = M.compatMode;
			if (Q || G) {
				P = (Q == "CSS1Compat") ? M.documentElement.clientWidth
						: M.body.clientWidth;
			}
			return P;
		},
		...
		getClientRegion : function() {
			var R = B.Dom.getDocumentScrollTop(), Q = B.Dom
					.getDocumentScrollLeft(), S = B.Dom.getViewportWidth() + Q, P = B.Dom
					.getViewportHeight()
					+ R;
	...
	var H = function() {
		if (M.documentElement.getBoundingClientRect) {
			return function(Q) {
				...
				return [ R.left + B.Dom.getDocumentScrollLeft(P),
			...
			return function(R) {
				...
				if (Q != R) {
					while (Q) {
						...
						S[1] += Q.offsetTop;
						if (!P && L
								&& B.Dom.getStyle(Q, "position") == "absolute") {
							P = true;
						}
						Q = Q.offsetParent;
					}
				...
				if (this.useLegacyEvent(L, K)) {
					...
					if (M) {
						for (O = 0, R = M.length; O < R; ++O) {
							V = M[O];
							if (V && V[this.EL] == L && V[this.TYPE] == K
									&& V[this.FN] == T) {
								M[O] = null;
								break;
							}
